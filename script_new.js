document.addEventListener('DOMContentLoaded', function () {
    const API_BASE_URL = "http://your-real-api-url.com/api/deployment"; // Replace with real API URL
    ...
    form.addEventListener('submit', async function (e) {
        ...
        for (let env of environmentsToCheck) {
            ...
            const apiUrl = `${API_BASE_URL}?bgflag=${bgflagValue}&environment=${env}`;
            ...
            try {
                ...
                if (result.status === "success" && result.data.length > 0) {
                    // Loop through the array and add multiple rows if needed
